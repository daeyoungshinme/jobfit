...
- [app/models.py](app/models.py) — SQLAlchemy 모델 (`JobPosting`, `Resume`, `ApplicationEvent`). 타임스탬프는 네이티브 UTC(`_now()`). `ApplicationEvent.job_id` 는 소프트 참조(FK 아님).
