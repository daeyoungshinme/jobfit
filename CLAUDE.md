...
- [app/services/matcher.py](app/services/matcher.py) — 이력서-공고 매칭 점수 계산. 가중치(`MATCH_REQUIRED_WEIGHT=0.7`, `MATCH_PREFERRED_WEIGHT=0.3`)는 [app/constants.py](app/constants.py)에 정의. `MatchConfig`(현재 `axes_weight` 단일 필드) + `DEFAULT_CONFIG`가 현행 재현. related 부분점수·필수/우대 가중은 `matcher.py` 상단 튜닝 상수(`RELATED_CREDIT` 등). `compute_match(..., *, config, skill_weights, resume)` — `skill_weights`(`scarcity_weights()`가 `skill_ranking` percentage 역수로 산출)는 희소 스킬 미보유에 더 큰 감점. `resume`가 주어지면 부가 축 2개도 계산: `experience_fit()`는 `total_years` vs 공고 `experience_level`, `position_fit()`는 `Resume.target_position` vs 공고 `position`(완전 일치 1.0 / `POSITION` meta `adjacent` 인접 직무 0.6 / 그 외 0.2, 한쪽이라도 없거나 `other`면 축 제외). `axes_weight=0` 기본이면 두 축 모두 투명 노출만, dashboard `?axes=1`이면 존재하는 부가 축들의 평균을 `axes_weight`로 점수에 반영. dashboard 라우터가 `scarcity_weights` 항상 주입.
...
- [app/models.py](app/models.py) — SQLAlchemy 모델 (`JobPosting`, `Resume`, `ApplicationEvent`). 타임스탬프는 네이티브 UTC(`_now()`). `ApplicationEvent.job_id` 는 소프트 참조(FK 아님). `Resume.total_years`/`target_position`(POSITION 코드)는 경력·직무 적합도 축 매칭용(`matcher.experience_fit`/`position_fit`) — 업로드 시 추측, 이력서 폼(`_resume_fields.html::career_meta_fields`)에서 수정.
- [app/db.py](app/db.py) — SQLite 엔진/세션(`engine`/`SessionLocal`/`Base`/`get_db`), 얇은 `init_db()`(= `create_all` + `migrations.run_migrations()`). 404는 `main.py`의 `StarletteHTTPException` 핸들러가 브라우저엔 `404.html`, API엔 JSON으로 응답.
- [app/migrations.py](app/migrations.py) — 스키마/데이터 마이그레이션 일체(아래 컨벤션 참고). `db.engine`/`db.SessionLocal`을 호출 시점에 읽어 테스트가 임시 DB로 갈아끼울 수 있다.
...
- **DB 마이그레이션** ([app/migrations.py](app/migrations.py), 전용 도구 없음): `_migrate_table_columns(table_name, columns)`가 테이블 무관 범용 헬퍼입니다 — `JobPosting`에 컬럼을 추가할 때는 `_JOB_POSTING_NEW_COLUMNS`에 `(컬럼명, DDL타입, 기본값 리터럴)`을 추가하면 `run_migrations()`가 누락된 컬럼만 `ALTER TABLE`로 추가합니다(멱등, 플래그 불필요). 1회성 데이터 마이그레이션은 `_run_data_migrations()`가 `schema_meta` 테이블의 스텝별 `"done"` 플래그로 가드해 **DB당 정확히 한 번**만 실행합니다(`_migrate_enum_codes` 라벨→코드, `_reparse_all_job_postings` 원문 재파싱+빈 필드 추측, `_backfill_updated_at`, `_backfill_resume_career`). 새 데이터 마이그레이션은 `_run_data_migrations()`에 `if _get_meta("...") != "done": ...; _set_meta("...", "done")` 스텝으로 추가하세요(버전 번호 없음). 행 단위 재파싱은 `try/except Exception` + `logger.warning`으로 격리돼 불량 `raw_text` 한 행이 startup 을 막지 못합니다. `Resume`도 같은 패턴(`_RESUME_NEW_COLUMNS` — `total_years`/`target_position`, 백필 `_backfill_resume_career()`가 `resume_career_backfilled` 플래그). 모델/서비스 import 는 `app.db` 로드 순서 때문에 함수 로컬로 둡니다.
